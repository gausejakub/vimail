...
// MockStore implements Store with hardcoded data for development/testing.
...
// NewMockStore creates a MockStore populated with sample data.
...
func (s *MockStore) Accounts() []Account {
	return s.accounts
}

func (s *MockStore) FoldersFor(email string) []Folder {
	if f, ok := s.folders[email]; ok {
		return f
	}
	return nil
}

func (s *MockStore) MessagesFor(email, folder string) []Message {
	if acct, ok := s.messages[email]; ok {
...
func (s *MockStore) DeleteDraft(email, id string) {
	acct, ok := s.messages[email]
	if !ok {
		return
	}
...
func (s *MockStore) NextDraftID() string {
	s.draftSeq++
	return fmt.Sprintf("draft-%d", s.draftSeq)
}

func (s *MockStore) MarkRead(email, folder, id string) {
	acct, ok := s.messages[email]
	if !ok {
		return
	}
	...
	for i, m := range msgs {
		if m.ID == id {
			...
			if fs, ok := s.folders[email]; ok {
				for j, f := range fs {
					if f.Name == folder && f.UnreadCount > 0 {
						fs[j].UnreadCount--
						break
					}
				}
...
func (s *MockStore) DeleteMessage(acctEmail, folder, id string) {
	acct, ok := s.messages[acctEmail]
	if !ok {
		return
	}
	msgs, ok := acct[folder]
	...
	for i, m := range msgs {
		if m.ID == id {
			if m.Unread {
				if fs, ok := s.folders[acctEmail]; ok {
					for j, f := range fs {
						if f.Name == folder && f.UnreadCount > 0 {
							fs[j].UnreadCount--
							break
			...
			acct[folder] = append(msgs[:i], msgs[i+1:]...)
			// Move to Trash (unless already in Trash — then permanently delete).
			if folder != "Trash" {
				m.Unread = false
				acct["Trash"] = append(acct["Trash"], m)
			}
			return
		}
	}
