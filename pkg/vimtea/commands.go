...
func registerBindings(m *editorModel) {
	...
	// diw/ciw/yiw now registered via registerTextObjectBindings in operators.go
...
func moveToStartOfDocument(model *editorModel) tea.Cmd {
	if model.countPrefix > 0 {
		// 9gg → go to line 9 (1-indexed → 0-indexed)
		target := model.countPrefix - 1
		if target >= model.buffer.lineCount() {
			target = model.buffer.lineCount() - 1
		}
		model.cursor.Row = target
	} else {
		model.cursor.Row = 0
	}
	model.cursor.Col = min(model.desiredCol, max(0, model.buffer.lineLength(model.cursor.Row)-1))
...
func moveToEndOfDocument(model *editorModel) tea.Cmd {
	if model.countPrefix > 0 {
		// 9G → go to line 9 (1-indexed → 0-indexed)
		target := model.countPrefix - 1
		if target >= model.buffer.lineCount() {
			target = model.buffer.lineCount() - 1
		}
...
// performWordOperation, deleteInnerWord, yankInnerWord, changeInnerWord
// have been replaced by the table-driven text objects system in textobjects.go.
