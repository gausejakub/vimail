...
type editorModel struct {
	...
	lastFindAction string // "f","F","t","T" (for ;/,)

	// Search state
	searchMode    bool   // collecting search input
	searchForward bool   // true=/, false=?
	searchBuffer  string // search input being typed
	searchPattern string // last executed search pattern
...
func (m *editorModel) handleKeypress(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	// Intercept pending actions (f/F/t/T/r and operator+find combos)
	if m.pendingAction != "" {
		return m.handlePendingAction(msg)
	}
	// Intercept search mode input
	if m.searchMode {
